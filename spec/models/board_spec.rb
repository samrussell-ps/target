...
RSpec.describe Board, type: :model do
  it 'accepts with a valid DictionaryEntry and seed' do
    expect(Board.create(dictionary_entry: DictionaryEntry.order("RANDOM()").first,
          word_shuffle_seed: 234)).to be_valid
  end

  it 'does not accept with a valid DictionaryEntry and no seed' do
    expect(Board.create(dictionary_entry: DictionaryEntry.order("RANDOM()").first,
          word_shuffle_seed: nil)).to_not be_valid
  end
  ...
  it 'destroys submitted_words when the model is destroyed' do
    board = Board.create!(dictionary_entry: DictionaryEntry.order("RANDOM()").first, word_shuffle_seed: 234)
